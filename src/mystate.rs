use crate::objects::{BandwidthUnitFamily, Connection, LocalSocket, ProcessInfo, Utilization};
use std::{
    cmp,
                    ...
                    if proc_info.is_none() && !self.known_orphan_sockets.contains(&local_socket) {
                        ...
                        match connections_to_procs
                            ...
                            Some((lookalike, proc_info)) => {
                                ...
                                println!("Looking for: {connection:?}; found: {lookalike:?}");
                            }
                            None => {
                                println!("Cannot determine which process owns {connection:?}");
                            }
                        };
                    }
